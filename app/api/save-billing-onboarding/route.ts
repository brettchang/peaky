...
import { getClientByPortalId, getCampaignById, saveBillingOnboardingForm } from "@/lib/db";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const ok = await saveBillingOnboardingForm(campaignId, {
      primaryContactName,
      primaryContactEmail,
      representingClient,
      wantsPeakCopy,
    ...
    if (!ok) {
      return NextResponse.json(
        { error: "Failed to save billing onboarding" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to save billing onboarding" },
      { status: 400 }
    );
  }
}
