...
export function OnboardingStatus({
  ...
  return (
    <div className="mb-8 space-y-3">
      ...
      {/* Native onboarding: waiting on client */}
      {!onboardingSubmittedAt && rounds.length === 0 && (
          ...
          <div className="flex items-center justify-between px-5 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Client needs to fill out the onboarding form via their portal link
                </p>
              </div>
            </div>
            {portalUrl && (
              <button
                onClick={() => handleCopy("portal", `${portalUrl}/${campaignId}`)}
                className="rounded-lg border border-amber-300 bg-white px-4 py-2 text-sm font-medium text-amber-800 hover:bg-amber-100"
              >
                {copiedId === "portal" ? "Copied!" : "Copy Portal Link"}
              </button>
            )}
          </div>
        </div>
      ...
      {/* Legacy Fillout rounds (for campaigns that already have them) */}
      {rounds.map((round) => {
      ...
      {/* Only show "+ New Round" for campaigns that already use Fillout */}
      {rounds.length > 0 && (
        <>
          {showNewRound ? (
